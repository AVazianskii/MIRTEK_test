...
{
    internal class Production_year_handler : Handler_template
    {
        public override object Handle(byte[] cmd_byte_array, List<Car_template> auto_park, Car_template Car, List<byte> byte_array)
        {
            if ((cmd_byte_array[0] == 1) | (cmd_byte_array[0] == 2) | ((cmd_byte_array[0] == 3) & (cmd_byte_array[2] == 2)))
            {
                    byte_array.Add(0x12);                                               // добавляем признак целого числа без знака
                    if(Car.Get_Production_year() != null)
                    {
                        foreach(byte element in BitConverter.GetBytes((int)Car.Get_Production_year()))
                        {
